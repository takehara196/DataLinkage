...
def split_fld_param(collection_df):
    ...
    #     tmp_df = fld_param_df[col_list]
    #     print(type(tmp_df))
    #     for c in range(1):
    #         print(tmp_df.iloc[:, c])
            # print(tmp_df.iloc[:, c + 1])
            # print(tmp_df.iloc[:, c].str.cat(tmp_df.iloc[:, c + 1]))

        # for c in range(len(col_list)):
        #     tmp_df = pd.DataFrame(tmp_df.iloc[:, c])
    ...
    for col_list in get_unique_list(col_list):
        ...
        for col in col_list:
            ...
            # サフィックスの数値が0以外であれば数値回だけ結合する
                ...
                merge_col_list = list(set(merge_col_list))
    print(f"merge_col_list: {merge_col_list}")

    for m in merge_col_list:
        print(m)
